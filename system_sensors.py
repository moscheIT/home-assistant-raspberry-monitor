...
broker_url = "123.123.123.123"#MQTT server IP
...
deviceName = "node1" #Name off your PI
...
def updateSensors():
    client.publish(topic="homeassistant/sensor/"+ deviceName +"/state", payload='{"temperature": '+ get_temp() +', "disk_use": '+ get_disk_usage() + ', "memory_use": '+ get_memory_usage() +', "cpu_usage": '+ get_cpu_usage() + ', "power_status": "'+ get_rpi_power_status() +'", "last_boot": "'+ get_last_boot() +'", "wifi": '+ get_wifi() +' }', qos=1, retain=False)
...
def get_wifi():
    ...
    for line in cmd.stdout:
        ...
        if "Link Quality" in line:
            outwifi=re.split(r'\s+|=',line)
            # print (outwifi[3].lstrip()) quality
            return outwifi[6].lstrip()
...
if __name__ == "__main__":
    ...
    client.publish(topic="homeassistant/sensor/"+ deviceName +"/"+ deviceName +"WIFI/config", payload='{ "name": "'+ deviceName +'Wifi", "state_topic": "homeassistant/sensor/'+ deviceName +'/state", "unit_of_measurement": "dbm", "value_template": "{{ value_json.wifi}}" }', qos=1, retain=True)
